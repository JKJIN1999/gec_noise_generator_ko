...
def noise(data_directory, error_list, result_directory, json_maximum, tokenizer_type, label_type, error_by):
    error_count_dic = {"OM":0,"REP":0,"MIF":0, "ADD":0, "S_ADD":0, "S_DEL":0}
    file_num = 0
    last_line = 0
    result = []
    ...
    for sentence_num in range(0, len(sentences)):
        ...
        for error_type in error_list:
            try:
                ...
                message = (" 오류 유형 {} 을 {} 번째 문장에 생성도중 오류가 발생했습니다... 다음 문장으로 넘어갑니다".format(error_type, sentence_num))
        ...
        indices = sorted(random.sample(range(len(conv_list)), max_error)) if len(conv_list) > max_error else range(len(conv_list))
        ...
        if sentence_num > 0 and (sentence_num+1) % json_maximum == 0:
            ...
            current_time = int((len(sentences)/sentence_num)*int(time.time()-start_time))
            ...
            message = (" 문장 {} ~ {} 까지 오류가 생성되었고 json 리스트를 새로운 파일 {} 에 저장했습니다.\n 모든 문장에 오류 생성까지 : {} 초 예상됩니다".format(sentence_num - json_maximum + 1, sentence_num, file_name,avg_time))
            logger.info(message)
            print(message)
            result = []
    ...
    if label_type == "benchmark":
        ...
        for e_type, e_count in error_count_dic.items():
            message = ([" 오류 유형 {} 에 생성된 오류 총량은 : {} 입니다".format(e_type, e_count)," 오류 생성 확률은 : {} 입니다".format(__avg_cal(e_count,total_amount))])
